...
// Find the _showConnectDialog method in maps_screen.dart and replace it with this implementation
  Future<void> _showConnectDialog() async {
    ...
    if (_userRole == 'caregiver') {
      ...
      if (!emailRegExp.hasMatch(result)) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Invalid email format: $result'),
            backgroundColor: Colors.red,
          ),
        );
        setState(() {
      ...
      try {
        // Attempt to connect with patient
        final Map<String, dynamic> connectionResult =
        await _locationService.connectWithPatient(result);
        ...
        if (mounted) {
          if (connectionResult['success']) {
            ...
            setState(() {
              _isConnected = true;
              _connectedPatientId = connectionResult['patientId'];
              _connectedPatientEmail = connectionResult['patientEmail'];
              _isConnecting = false;
            });

            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text(connectionResult['message']),
                backgroundColor: Colors.green,
              ),
            );

            // Start tracking patient if we have their ID
            if (_connectedPatientId != null) {
              _startTrackingPatient();
            }
          } else {
        ...
        print("Error in connect dialog: $e");
        if (mounted) {
          setState(() {
            _isConnecting = false;
          });
