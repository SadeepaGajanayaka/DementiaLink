...
// Helper function to handle different asset path formats
String normalizeAssetPath(String assetPath) {
  if (assetPath.startsWith('lib/assets/')) {
    return assetPath.replaceFirst('lib/', '');
  }
  return assetPath;
}
...
class SongService {
  ...
  final String baseUrl = 'http://192.168.1.43:3003'; // Updated port to 3003
  ...
  Future<Song> downloadSongFiles(Song song) async {
    // Skip asset files
    if (song.audioPath.startsWith('assets/') || song.audioPath.startsWith('lib/assets/')) {
      return song;
    }
...
class UploadScreen extends StatefulWidget {
  ...
  final String? appBarBackgroundColor; // Add custom color option
...
class _UploadScreenState extends State<UploadScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final backgroundColor = widget.appBarBackgroundColor != null
        ? Color(int.parse(widget.appBarBackgroundColor!.replaceAll('#', '0xFF')))
...
class PlaylistScreen extends StatefulWidget {
  // Add optional parameters for integration with dashboard
...
class PlaylistScreenState extends State<PlaylistScreen> {
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) => UploadScreen(
          ...
          appBarBackgroundColor: widget.customColor != null
              ? '#${widget.customColor!.value.toRadixString(16).substring(2)}'
              : null,
        ),
      ),
    );
    ...
    try {
      ...
      for (final song in initialSongs) {
        try {
          ...
          final imageData = await rootBundle.load(normalizeAssetPath(song.imagePath));
          ...
          final audioData = await rootBundle.load(normalizeAssetPath(song.audioPath));
  ...
  Widget build(BuildContext context) {
    // Use custom color if provided, otherwise default
    final backgroundColor = widget.customColor ?? const Color(0xFF503663);
  ...
  Widget _buildHeader() {
    return Padding(
      ...
      onPressed: () {
        // Use custom back handler if provided, otherwise default behavior
        if (widget.onBackPressed != null && widget.fromDashboard) {
    ...
    child: ClipOval(
    child: Image.asset(
    normalizeAssetPath('assets/images/brain_icon.png'),
    fit: BoxFit.cover,
  ...
  Widget _buildSongTile(BuildContext context, Song song, int index) {
    ...
    Widget buildImage() {
      if (song.imagePath.startsWith('assets/') || song.imagePath.startsWith('lib/assets/')) {
        return Image.asset(
          normalizeAssetPath(song.imagePath),
          width: 48,
          height: 48,
          fit: BoxFit.cover,
          errorBuilder: (context, error, stackTrace) {
...
// PlayerScreen class implementation - this is where the music is played
...
class PlayerScreenState extends State<PlayerScreen> {
  ...
  Future<void> _loadCurrentSong() async {
    ...
    try {
      ...
      if (_currentSong.audioPath.startsWith('assets/') || _currentSong.audioPath.startsWith('lib/assets/')) {
        await _audioPlayer.setAsset(normalizeAssetPath(_currentSong.audioPath));
      } else {
  ...
  Widget build(BuildContext context) {
    // Use custom color if provided, otherwise default
    final backgroundColor = widget.customColor ?? const Color(0xFF4A2B5C);
  ...
  Widget _buildAlbumArt() {
    Widget buildImage() {
      ...
      } else if (_currentSong.imagePath.startsWith('assets/') || _currentSong.imagePath.startsWith('lib/assets/')) {
        return Image.asset(
          normalizeAssetPath(_currentSong.imagePath),
          fit: BoxFit.cover,
          errorBuilder: (context, error, stackTrace) {
            return Container(
...
// This class can be used to connect the music player to the dashboard
class MusicDashboardConnector {
  // Method to navigate from dashboard to music player
  static void openMusicPlayerFromDashboard(BuildContext context) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => const PlaylistScreen(
          fromDashboard: true,
          customColor: Color(0xFF503663),  // Match the dashboard color scheme
        ),
      ),
    );
  }
...
// Main app class for standalone testing
...
// If you're using this as a standalone app for testing
void main() {
  runApp(const MyApp());
}