...
// Find the _connectWithPatient method in connect.dart and replace with this implementation
  Future<void> _connectWithPatient() async {
    ...
    try {
      final email = _emailController.text.trim();
      print("Attempting to connect with patient email: $email");

      final result = await _locationService.connectWithPatient(email);

      if (mounted) {
        if (result['success']) {
          // Success - Return to maps screen with the patient email
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Successfully connected with $email'),
          ...
          SystemChrome.setEnabledSystemUIMode(SystemUiMode.edgeToEdge);
          Navigator.pop(context, email); // Return the email instead of patient ID
        } else {
          // Error
          setState(() {
